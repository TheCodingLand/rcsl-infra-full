...
def modifyobjref(id, name, value):
    ...
    if XmlResponse.result == "false":
        print XmlResponse.xml.encode(Encoding)
...
def list_categories():
    result = ""
    for cat in AVAILABLE_CATEGORIES.iterkeys():
        result = "%s %s," %(result, cat.decode('iso-8859-1').encode(Encoding))
    result = result[1:-1]
    result = "[%s]" % result
    print "%s" % (result)
...
def classement(username):
    ...
    print u"voici la liste des tickets à classer :".encode(Encoding)
    for ticket in tickets:
        print ticket.id.encode(Encoding) + ": " + ticket.title.encode(Encoding)
    print u"id tu ticket à classer ?".encode(Encoding)
...
def searchTicket(text, username, solved):
    print "searching for %s".encode(Encoding) % text
    ...
    for child in root:
        if child.attrib['id']:
            ...
            if solved =="True":
                try:
                    print testresult.encode(Encoding)
                except Exception, e:
                    print e
...
def closeTicket(id, username, solution):
    ...
    if currentsolution == "None":
        SolutionDescription = "Alfred : %s" % solution.encode(Encoding)
    else:
    ...
    title = result.title.encode(Encoding)
    print "Title : %s" % (title)
...
def findAdmintickets(username):
    ...
    for child in root:
        try:
            ...
            print "got an exception for some ticket"
